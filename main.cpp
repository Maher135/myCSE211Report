...
// ISR for S3 press (falling edge = button pressed)
void onS3Press() {
    showVoltage = true;
}

// ISR for S3 release (rising edge = button released)
void onS3Release() {
    showVoltage = false;
}
...
void updateDisplay() {
    ...
    if (showVoltage) {
        ...
        digits[0] = volt % 10;           // hundredths
        digits[1] = (volt / 10) % 10;    // tenths
        digits[2] = (volt / 100) % 10;   // whole volts
        digits[3] = 0;                   // optional blank or zero
    } else {
        if (counting)
...
int main() {
    ...
    s3_button.rise(&onS3Release); // New addition for release handling

    timer.reset();
    displayTicker.attach_us(&updateDisplay, 3000); // every 3ms
    ...
    while (true) {
        ...
        ThisThread::sleep_for(100ms); // avoid CPU overuse
    }
}
